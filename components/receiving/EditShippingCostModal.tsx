...
interface Trip {
  ...
  // จุดส่งพิเศษที่ไม่มี order
  extra_delivery_stops?: ExtraDeliveryStop[];
}
...
interface TripFormData {
  ...
  extra_delivery_stops: Array<ExtraDeliveryStop>;
  actual_stops_count?: number | null; // จำนวนจุดส่งจริง (กรณีหลายร้านส่งที่เดียวกัน)
}

// จุดส่งพิเศษที่ไม่มี order เช่น ถุง ของแถม ของขวัญ
interface ExtraDeliveryStop {
  name: string;
  description: string;
  cost: number;
}
...
const TripEditForm: React.FC<TripEditFormProps> = ({ trip, tripIndex, suppliers, onDataChange }) => {
  ...
  // State for extra delivery stops (จุดส่งพิเศษที่ไม่มี order เช่น ถุง ของแถม ของขวัญ)
  const [extraDeliveryStops, setExtraDeliveryStops] = useState<Array<ExtraDeliveryStop>>(
    (trip as any).extra_delivery_stops || []
  );

  // State for actual stops count override (กรณีหลายร้านส่งที่เดียวกัน)
  const [actualStopsCount, setActualStopsCount] = useState<number | null>(
    (trip as any).actual_stops_count || null
  );
  ...
  useEffect(() => {
    onDataChange(trip.trip_id, {
      ...
      extra_delivery_stops: extraDeliveryStops,
      orderRemarks,
      actual_stops_count: actualStopsCount
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [formData, pricingMode, otherFees, extraDeliveryStops, orderRemarks, actualStopsCount, trip.trip_id]);
  ...
  const calculatedStops = uniqueCustomerIds.size;
  // ใช้ actual_stops_count ถ้ามีการระบุ (กรณีหลายร้านส่งที่เดียวกัน)
  const totalStops = actualStopsCount !== null && actualStopsCount > 0 ? actualStopsCount : calculatedStops;
    ...
    <div className={`border-2 rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow ${
      ...
      <div className="mb-4">
        <h3 className="text-base font-bold text-gray-900 font-thai mb-3">
          ...
          <span className="ml-2 text-xs text-gray-500 font-normal">(เที่ยวที่ {trip.trip_sequence} ในแผน)</span>
          {trip.needs_shipping_cost_update && (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2 text-sm mb-4">
          ...
          <div className="flex flex-col py-1.5 px-2 bg-gray-50 rounded">
            <div className="flex justify-between items-center">
              <span className="text-gray-600 text-xs">จุดส่ง (ลูกค้า):</span>
              <span className="font-semibold text-gray-900">{calculatedStops} จุด</span>
            </div>
            {/* ช่องกรอกจำนวนจุดส่งจริง */}
            <div className="flex items-center gap-1 mt-1">
              <span className="text-gray-500 text-xs">จุดจริง:</span>
              <input
                type="number"
                min="1"
                max={calculatedStops}
                value={actualStopsCount || ''}
                onChange={(e) => {
                  const val = e.target.value ? parseInt(e.target.value) : null;
                  setActualStopsCount(val);
                }}
                placeholder={String(calculatedStops)}
                ...
                title="กรณีหลายร้านส่งที่เดียวกัน ให้ระบุจำนวนจุดส่งจริง"
              />
              {actualStopsCount !== null && actualStopsCount !== calculatedStops && (
                <button
                  type="button"
                  onClick={() => setActualStopsCount(null)}
                  className="text-red-500 hover:text-red-700 text-xs"
                  title="รีเซ็ตเป็นค่าเดิม"
                >
                  ✕
                </button>
              )}
      ...
      {/* Main Info Table */}
      <table className="w-full mb-3 text-sm border-collapse">
        <tbody>
          <tr className="border-b border-gray-200">
            <td className="py-2 pr-2 text-gray-600 font-thai w-28">ผู้ให้บริการ</td>
            <td className="py-2">
              <div className="flex gap-1">
                <select
                  value={formData.supplier_id}
                  onChange={(e) => handleChange('supplier_id', e.target.value)}
                  className="flex-1 px-2 py-1.5 text-sm border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 font-thai bg-white"
                >
                ...
                <button
                  type="button"
                  onClick={() => window.open('/master-data/suppliers', '_blank')}
                  className="px-2 py-1.5 text-sm bg-green-500 text-white rounded hover:bg-green-600"
          ...
          <tr className="border-b border-gray-200">
            <td className="py-2 pr-2 text-gray-600 font-thai">ทะเบียนรถ</td>
            <td className="py-2">
              {!formData.supplier_id ? (
                <span className="text-gray-400 text-xs font-thai">เลือกผู้ให้บริการก่อน</span>
              ) : loadingVehicles ? (
                <span className="text-gray-400 text-xs font-thai">กำลังโหลด...</span>
              ) : vehicles.length === 0 ? (
                <span className="text-yellow-600 text-xs font-thai">ไม่มีรถ</span>
              ) : (
                <select
                  ...
                  className="w-full px-2 py-1.5 text-sm border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 font-thai bg-white"
                >
          ...
          <tr className="border-b border-gray-200">
            <td className="py-2 pr-2 text-gray-600 font-thai">ผู้ขับ</td>
            <td className="py-2">
              <span className="text-gray-700 font-thai">{formData.driver_label || '-'}</span>
            </td>
          </tr>
          <tr className="border-b border-gray-200">
            <td className="py-2 pr-2 text-gray-600 font-thai">โหมดราคา</td>
            <td className="py-2">
              <div className="flex gap-4">
                <label className="flex items-center cursor-pointer">
                  <input
                    ...
                    className="mr-1.5"
                  />
                  <span className="text-sm font-thai">คำนวณ</span>
                </label>
                <label className="flex items-center cursor-pointer">
                  <input
                    type="radio"
                    ...
                    className="mr-1.5"
                  />
                  <span className="text-sm font-thai">เหมา</span>
                </label>
              </div>
            </td>
          </tr>
      ...
      {pricingMode === 'flat' ? (
        <table className="w-full mb-3 text-sm border-collapse bg-blue-50 rounded">
          <tbody>
            <tr className="border-b border-blue-200">
              <td className="py-2 px-2 text-gray-600 font-thai w-28">ค่าเหมา <span className="text-red-500">*</span></td>
              <td className="py-2 px-2">
                <input
                  type="number"
                  value={formData.shipping_cost}
                  onChange={(e) => handleChange('shipping_cost', parseFloat(e.target.value) || 0)}
                  className="w-32 px-2 py-1.5 text-sm border-2 border-yellow-400 rounded focus:outline-none focus:ring-2 focus:ring-yellow-500 font-semibold bg-yellow-50"
                  placeholder="0"
                />
            ...
            <tr className="border-b border-blue-200">
              <td className="py-2 px-2 text-gray-600 font-thai">ค่าแบก</td>
              <td className="py-2 px-2">
                <input
                  type="number"
                  value={formData.porterage_fee}
                  onChange={(e) => handleChange('porterage_fee', parseFloat(e.target.value) || 0)}
                  className="w-32 px-2 py-1.5 text-sm border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                  placeholder="0"
                />
                <span className="ml-1 text-gray-500">฿</span>
              </td>
        ...
        <table className="w-full mb-3 text-sm border-collapse bg-green-50 rounded">
          <tbody>
            <tr className="border-b border-green-200">
              <td className="py-2 px-2 text-gray-600 font-thai w-28">ราคาเริ่มต้น <span className="text-red-500">*</span></td>
              <td className="py-2 px-2">
                <input
                  type="number"
                  ...
                  className="w-24 px-2 py-1.5 text-sm border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-green-500 bg-white"
                  placeholder="0"
                />
                <span className="ml-1 text-gray-500">฿</span>
              </td>
            ...
            <tr className="border-b border-green-200">
              <td className="py-2 px-2 text-gray-600 font-thai">ค่าเด็ก</td>
              <td className="py-2 px-2">
                <input
                  type="number"
            ...
            <tr className="border-b border-green-200">
              <td className="py-2 px-2 text-gray-600 font-thai">ค่าจุดเพิ่ม</td>
              <td className="py-2 px-2">
                <input
                  type="number"
                  ...
                  placeholder="0"
                />
                <span className="ml-1 text-gray-500">฿/จุด</span>
                <span className="ml-2 text-xs text-gray-400">({Math.max(0, totalStops - 1)} จุด = {(Math.max(0, totalStops - 1) * formData.extra_stop_fee).toLocaleString()} ฿)</span>
              </td>
            </tr>
            <tr className="border-b border-green-200">
              <td className="py-2 px-2 text-gray-600 font-thai">ค่าแบก</td>
            ...
            <tr className="bg-green-100">
              <td className="py-2 px-2 text-gray-700 font-thai font-semibold">รวม</td>
              <td className="py-2 px-2 text-green-700 font-bold">{calculateShippingCost().toLocaleString()} ฿</td>
            </tr>
          </tbody>
        </table>
      )}

      {/* Other Fees & Extra Stops */}
      <div className="mb-3 p-2 bg-gray-50 rounded border border-gray-200">
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs font-medium text-gray-600 font-thai">ค่าใช้จ่ายอื่นๆ</span>
          <button
            type="button"
            onClick={() => setOtherFees([...otherFees, { label: '', amount: 0 }])}
            className="px-2 py-1 bg-blue-500 text-white rounded text-xs hover:bg-blue-600 font-thai"
          >+ เพิ่ม</button>
        </div>
        {otherFees.length > 0 && (
                ...
                <tr key={index} className="border-b border-gray-200">
                  <td className="py-1">
                    <input
                      ...
                      className="w-full px-2 py-1 text-sm border border-gray-300 rounded font-thai bg-white"
                      placeholder="รายการ"
                    />
                  </td>
                  ...
                  <td className="py-1 w-8 text-center">
                    <button
                      type="button"
                      onClick={() => setOtherFees(otherFees.filter((_, i) => i !== index))}
        ...
        <div className="mt-2">
          <button
            ...
            disabled={savingToMaster || !formData.supplier_id || (pricingMode === 'flat' ? formData.shipping_cost <= 0 : formData.base_price <= 0)}
            className="px-2 py-1 bg-purple-500 text-white rounded text-xs hover:bg-purple-600 disabled:bg-gray-400 font-thai"
          >{savingToMaster ? 'กำลังบันทึก...' : 'บันทึกเข้ามาสเตอร์'}</button>
        </div>
      </div>
      ...
      <div className="mb-3 p-2 bg-orange-50 rounded border border-orange-200">
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs font-medium text-gray-600 font-thai">🎁 จุดส่งพิเศษ (ไม่มี Order)</span>
          <button
            type="button"
            onClick={() => setExtraDeliveryStops([...extraDeliveryStops, { name: '', description: '', cost: 0 }])}
            className="px-2 py-1 bg-orange-500 text-white rounded text-xs hover:bg-orange-600 font-thai"
          >+ เพิ่ม</button>
        </div>
        {extraDeliveryStops.length > 0 && (
          <table className="w-full text-sm">
            <tbody>
              {extraDeliveryStops.map((stop, index) => (
                <tr key={index} className="border-b border-orange-200">
                  <td className="py-1">
                    <input
                      ...
                      value={stop.name}
                      onChange={(e) => {
                        const newStops = [...extraDeliveryStops];
                        newStops[index].name = e.target.value;
                        setExtraDeliveryStops(newStops);
                      }}
                      className="w-full px-2 py-1 text-sm border border-gray-300 rounded font-thai bg-white"
                      placeholder="ชื่อจุดส่ง"
                    />
                  </td>
                  ...
                  <td className="py-1 w-20">
                    <input
                      ...
                      onChange={(e) => {
                        const newStops = [...extraDeliveryStops];
                        newStops[index].cost = parseFloat(e.target.value) || 0;
                        setExtraDeliveryStops(newStops);
                      }}
                      className="w-full px-2 py-1 text-sm border border-gray-300 rounded bg-white"
                      placeholder="0"
                  ...
                  <td className="py-1 w-8 text-center">
                    <button
                      type="button"
                      onClick={() => setExtraDeliveryStops(extraDeliveryStops.filter((_, i) => i !== index))}
                      className="text-red-500 hover:text-red-700"
                    >×</button>
                  </td>
                </tr>
      ...
      {/* Formula Mode - remove duplicate sections that were in the old code */}
      {pricingMode === 'formula' && null}
...
const EditShippingCostModal: React.FC<EditShippingCostModalProps> = ({
  ...
  // ถ้ามี planId ให้โหลดแผนนั้นโดยตรงและข้ามไปที่ step 'edit'
  useEffect(() => {
    if (isOpen && planId) {
      loadPlanDirectly(planId);
      fetchSuppliers();
    } else if (isOpen && step === 'select') {
      fetchPublishedPlans();
      fetchSuppliers();
    }
  }, [isOpen, planId]);
  ...
  const handleBack = () => {
    // ถ้ามี planId ให้ปิด modal แทนที่จะกลับไปหน้า select
    if (planId) {
      handleClose();
    } else {
      setStep('select');
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      const savePromises = selectedPlan.trips.map(async (trip) => {
        ...
        const extra_delivery_stops = tripData?.extra_delivery_stops ?? (trip as any).extra_delivery_stops ?? [];
        ...
        try {
          ...
          // จำนวนจุดส่ง = จำนวนรหัสลูกค้าที่ไม่ซ้ำกัน (1 จุด = 1 customer_id)
          // หรือใช้ actual_stops_count ถ้ามีการระบุ (กรณีหลายร้านส่งที่เดียวกัน)
          ...
          const calculatedStops = uniqueCustomerIdsForTrip.size;
          const actualStopsOverride = tripData?.actual_stops_count ?? (trip as any).actual_stops_count ?? null;
          const totalStops = actualStopsOverride !== null && actualStopsOverride > 0 ? actualStopsOverride : calculatedStops;
          ...
          const payload: any = {
            ...
            vehicle_id: vehicleId,
            driver_id: driverId,
            // porterage_fee, other_fees, and extra_delivery_stops should be saved regardless of pricing mode
            porterage_fee: porterage_fee,
            other_fees: other_fees,
            extra_delivery_stops: extra_delivery_stops,
            // actual_stops_count สำหรับกรณีหลายร้านส่งที่เดียวกัน
            actual_stops_count: actualStopsOverride,
