...
const ImportVehicleForm: React.FC<ImportVehicleFormProps> = ({ onSuccess, onCancel }) => {
  ...
  const downloadTemplate = () => {
    const csvContent = [
      'vehicle_id,vehicle_code,plate_number,vehicle_type,brand,model,year_of_manufacture,capacity_kg,capacity_cbm,fuel_type,current_status,remarks,created_by',
      'VH001,VH001,กก1234,รถบรรทุก,Toyota,Hilux Revo,2023,1000,2.5,Diesel,Active,รถใหม่,admin'
    ].join('\n');
    ...
    reader.onload = async (event) => {
      ...
      try {
        ...
        const vehicles = lines.slice(1).map(line => {
          ...
          const vehicle = header.reduce((obj, nextKey, index) => {
        ...
        const response = await fetch('/api/master-vehicle/import', {
          method: 'POST',
          headers: {
    ...
    <div className="space-y-4">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="bg-white/80 backdrop-blur-sm border border-white/20 rounded-2xl p-6 space-y-4 shadow-sm">
          <label className="block text-sm font-medium text-thai-gray-700 font-thai">เลือกไฟล์ข้อมูลยานพาหนะ</label>
          <div
