...
import { CreateLocationData, MasterWarehouse } from '@/types/database/warehouse';
...
const AddLocationForm: React.FC<AddLocationFormProps> = ({ onSuccess, onCancel, initialData }) => {
  ...
  const [formData, setFormData] = useState<Partial<CreateLocationData>>({
    ...
    location_type: initialData?.location_type || 'rack',
    max_capacity_qty: initialData?.max_capacity_qty || undefined,
    max_capacity_weight_kg: initialData?.max_capacity_weight_kg || undefined,
    current_qty: initialData?.current_qty || 0,
    current_weight_kg: initialData?.current_weight_kg || 0,
    putaway_strategy: initialData?.putaway_strategy || '',
    ...
    humidity_controlled: initialData?.humidity_controlled || false,
    active_status: initialData?.active_status || 'active',
    created_by: initialData?.created_by || 'admin', // TODO: Get from auth context
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await locationService.createLocation(formData as CreateLocationData);
      ...
      setError(err instanceof Error ? err.message : 'เกิดข้อผิดพลาดในการเพิ่มโลเคชั่น');
    } finally {
      setLoading(false);
    }
  };
