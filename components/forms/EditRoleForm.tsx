...
import { X, Shield, AlertCircle, Check, Eye, PlusCircle, Pencil, Trash2 } from 'lucide-react';
import Button from '@/components/ui/Button';
import { PermissionModule, SystemRoleWithPermissions, CreateRoleData } from '@/types/user-management-schema';

interface EditRoleFormProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (roleData: CreateRoleData) => Promise<void>;
  initialData: SystemRoleWithPermissions;
}

const EditRoleForm: React.FC<EditRoleFormProps> = ({ isOpen, onClose, onSubmit, initialData }) => {
  ...
  useEffect(() => {
    if (isOpen && initialData) {
      fetchModules();
      // Set initial form data from the role being edited
      setFormData({
  ...
  const fetchModules = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Initialize permissions for all modules, preserving existing ones
        const initialPermissions = data.map((module: PermissionModule) => {
          const existingPermission = initialData.permissions?.find(p => p.module_id === module.module_id);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-thai-gray-200">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-thai-gray-900 font-thai">แก้ไขบทบาท</h2>
              <p className="text-sm text-thai-gray-600 font-thai">แก้ไขข้อมูลบทบาทและสิทธิ์การเข้าถึง</p>
            </div>
          </div>
          <Button
            variant="ghost"
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="
                      ...
                      focus:outline-none focus:ring-2 focus:ring-yellow-500/50 focus:border-yellow-500/50 focus:bg-white
