...
const AddAssetForm: React.FC<AddAssetFormProps> = ({ onSuccess, onCancel }) => {
  ...
  const onSubmit = async (data: AssetFormValues) => {
    ...
    try {
      ...
      const response = await fetch('/api/master-asset', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        alert('เพิ่มข้อมูลทรัพย์สินสำเร็จ');
      ...
      // For now, simulate successful creation since API doesn't exist yet
      console.log('Asset data to be created:', data);
      alert('เพิ่มข้อมูลทรัพย์สินสำเร็จ (Demo Mode)');
      if (onSuccess) onSuccess();
    } finally {
      setLoading(false);
    }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <input
                {...register('asset_name')}
                id="asset_name"
                placeholder="เช่น Forklift Toyota 2.5T"
                className="
                  w-full px-4 py-3 bg-thai-gray-50/50 border border-thai-gray-200/50 rounded-xl
                  focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500/50 focus:bg-white/80
                  text-sm font-thai transition-all duration-300 backdrop-blur-sm
            ...
            <div className="md:col-span-2 space-y-2">
              ...
              <textarea
                ...
                id="description"
                rows={3}
                placeholder="ระบุรายละเอียดเพิ่มเติมเกี่ยวกับทรัพย์สิน..."
                className="
                  w-full px-4 py-3 bg-thai-gray-50/50 border border-thai-gray-200/50 rounded-xl
                  focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500/50 focus:bg-white/80
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <input
                {...register('brand')}
                id="brand"
                placeholder="เช่น Toyota, Crown, Zebra"
                className="
                  w-full px-4 py-3 bg-thai-gray-50/50 border border-thai-gray-200/50 rounded-xl
                  focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500/50 focus:bg-white/80
                  text-sm font-thai transition-all duration-300 backdrop-blur-sm
            ...
            <div className="space-y-2">
              ...
              <input
                {...register('model')}
                id="model"
                placeholder="เช่น 8FBE25, PTH50, TC26"
                className="
                  w-full px-4 py-3 bg-thai-gray-50/50 border border-thai-gray-200/50 rounded-xl
                  focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500/50 focus:bg-white/80
                  text-sm font-thai transition-all duration-300 backdrop-blur-sm
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                id="capacity_spec"
                placeholder="เช่น 2.5 Ton, 2000 Kg per level"
                className="
                  w-full px-4 py-3 bg-thai-gray-50/50 border border-thai-gray-200/50 rounded-xl
                  focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500/50 focus:bg-white/80
                  text-sm font-thai transition-all duration-300 backdrop-blur-sm
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <textarea
                ...
                rows={2}
                placeholder="เช่น ตรวจเช็ครายเดือน, บำรุงรักษารายปี"
                className="
                  w-full px-4 py-3 bg-thai-gray-50/50 border border-thai-gray-200/50 rounded-xl
                  focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500/50 focus:bg-white/80
                  text-sm font-thai transition-all duration-300 backdrop-blur-sm resize-none
