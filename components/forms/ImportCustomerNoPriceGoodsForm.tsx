...
interface ImportCustomerNoPriceGoodsFormProps {
  onSuccess?: () => void;
  onCancel: () => void;
}

const ImportCustomerNoPriceGoodsForm: React.FC<ImportCustomerNoPriceGoodsFormProps> = ({ onSuccess, onCancel }) => {
  ...
  const downloadTemplate = () => {
    const csvContent = `customer_id,customer_name,reason,note_for_picking,effective_start_date,effective_end_date,is_active,created_by
CUST001,ร้านค้าปลีกตัวอย่าง,ป้องกันการแสดงราคาให้ลูกค้าปลายทาง,⚠️ ห้ามวางสินค้าพร้อมสติ๊กเกอร์ราคา,2024-01-01,,true,system`;
    
    const blob = new Blob(['\uFEFF' + csvContent], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = 'customer_no_price_goods_template.csv';
    link.click();
  };
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/master-customer-no-price-goods/import', {
        method: 'POST',
        body: formData,
      });
    ...
    <div className="space-y-6">
      ...
      <div className="bg-yellow-50/50 backdrop-blur-sm border border-yellow-200/50 rounded-xl p-6">
        ...
        <ul className="text-xs text-yellow-700 space-y-1">
          ...
          <li>• คอลัมน์ที่จำเป็น: customer_id, customer_name, created_by</li>
          <li>• is_active ให้ใส่ true หรือ false</li>
          <li>• วันที่ให้ใส่รูปแบบ YYYY-MM-DD (เช่น 2024-01-01)</li>
...
export default ImportCustomerNoPriceGoodsForm;