...
export interface LoadingItemProgress {
  id: number;
  sku_id: string;
  sku_name: string;
  quantity: number;
...
interface LoadingProgressModalProps {
  isOpen: boolean;
  items: LoadingItemProgress[];
  totalItems: number;
  completedItems: number;
  currentItem?: LoadingItemProgress;
  hasError: boolean;
  errorItem?: LoadingItemProgress;
  onClose: () => void;
  onRetry?: () => void;
}
...
export default function LoadingProgressModal({
  ...
  const getStatusIcon = (status: LoadingItemProgress['status']) => {
    switch (status) {
      case 'pending':
  ...
  const getStatusText = (status: LoadingItemProgress['status']) => {
    switch (status) {
      case 'pending':
        return 'รอดำเนินการ';
      case 'checking':
        return 'กำลังตรวจสอบสต็อก...';
      case 'moving':
      ...
      case 'error':
        return 'สต็อกไม่พอ';
      case 'skipped':
        return 'ข้าม (สติ๊กเกอร์)';
      default:
        return '';
    }
  };

  const getStatusColor = (status: LoadingItemProgress['status']) => {
    switch (status) {
      case 'pending':
          ...
          {items.map((item, index) => (
            ...
            >
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[10px] text-gray-500 font-thai mt-0.5">
                    {item.sku_id} • {item.quantity} ชิ้น
                  </div>
                </div>
              </div>
              ...
              <div className="mt-2 ml-6">
                ...
                {/* Stock Check Info */}
                {item.status === 'checking' && item.available_qty !== undefined && (
                  <div className="text-[10px] text-blue-600 font-thai">
                    พบสต็อก {item.available_qty} ชิ้น
                  </div>
                )}
                ...
                {(item.status === 'moving' || item.status === 'success') && item.source_location && (
                  <div className="flex items-center gap-1 mt-1 text-[10px] text-gray-600 font-thai">
                    ...
                    <span>{item.dest_location || 'Delivery-In-Progress'}</span>
                  </div>
                )}
                ...
                {/* Skipped Details */}
                {item.status === 'skipped' && (
                  <div className="text-[10px] text-amber-600 font-thai mt-1">
                    ไม่ต้องย้ายสต็อก (ไม่ได้ track ในระบบ)
                  </div>
                )}
                ...
                {item.status === 'error' && item.error_details && (
                  <div className="mt-2 p-2 bg-red-100 rounded text-[10px] text-red-700 font-thai">
                    ...
                    {item.available_qty !== undefined && (
                      <div>• มีสต็อก: {item.available_qty} ชิ้น (ต้องการ {item.quantity})</div>
                    )}
                  </div>
                )}
              </div>
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <Package className="w-3 h-3 text-gray-400 flex-shrink-0" />
                    <span className="font-thai text-xs font-medium text-gray-900 truncate">
                      {currentItem.sku_name || currentItem.sku_id}
                    </span>
                  </div>
                  <div className="text-[10px] text-gray-500 font-thai mt-0.5">
                    {currentItem.sku_id} • {currentItem.quantity} ชิ้น
                  </div>
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1">
                <div className="font-thai text-sm font-medium text-red-700">
                  หยุดการทำงานเนื่องจากสต็อกไม่เพียงพอ
                </div>
                <div className="font-thai text-xs text-red-600 mt-1">
                  กรุณาตรวจสอบสต็อกที่ Dispatch แล้วลองใหม่
                </div>
              </div>
            </div>
          </div>
