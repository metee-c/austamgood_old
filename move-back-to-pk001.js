/**
 * ย้ายสินค้ากลับไปที่ PK001
 * สินค้าเหล่านี้ไม่มีบ้านหยิบเฉพาะ ควรอยู่ที่พื้นที่จัดสินค้า PK001
 */
...
// รายการสินค้าที่ต้องย้ายกลับไป PK001
const ITEMS = [
  ...
  { balance_id: 33494, sku_id: 'B-BEY-C|TUN|NS|010', from: 'A10-01-006', to: 'A09-01-005' }, // ตัวนี้ไปที่บ้านหยิบเฉพาะ
  { balance_id: 33640, sku_id: 'B-BAP-C|IND|030', from: 'A10-01-019', to: 'PK001' },
  { balance_id: 32127, sku_id: 'B-NET-C|FHC|040', from: 'A10-01-021', to: 'PK001' }
]

async function main() {
  console.log('🔧 กำลังย้ายสินค้ากลับไปที่ตำแหน่งที่ถูกต้อง...\n')
    ...
    try {
      ...
      const { data: move } = await supabase
        ...
        .insert({
          ...
          move_type: 'transfer',
          status: 'completed',
          notes: `ย้ายกลับไปตำแหน่งที่ถูกต้อง - Balance ID: ${item.balance_id}`,
  ...
  console.log(`⚠️  ข้ามไป: ${alreadyMovedCount} รายการ`)
