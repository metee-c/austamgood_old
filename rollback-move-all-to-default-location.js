/**
 * Rollback สคริปต์ move-all-to-default-location.js ที่รันผิด
 * ย้ายสินค้ากลับไปที่เดิมตามบันทึกใน wms_moves
 */
...
// ช่วง move_no ที่ต้อง rollback (จาก log ที่ให้มา)
const START_MOVE_NO = 'MV0000000320'
const END_MOVE_NO = 'MV0000000392' // อาจต้องปรับตามจำนวนจริง

async function main() {
  console.log('🔄 กำลัง Rollback การย้ายสินค้าที่ผิดพลาด...\n')
  ...
  const { data: moves, error: movesError } = await supabase
    ...
    `)
    .ilike('notes', '%ย้ายให้ตรงกับ default_location - Balance ID:%')
    .eq('status', 'completed')
  ...
  for (const move of moves) {
    ...
    for (const item of items) {
      ...
      try {
        ...
        if (parseFloat(currentBalance.total_piece_qty || 0) < parseFloat(item.confirmed_piece_qty || 0)) {
          console.log(`   ⚠️  ข้าม: สต็อกไม่เพียงพอที่จะ rollback\n`)
          skippedCount++
          continue
        }
        ...
        const { data: rollbackMove } = await supabase
          .from('wms_moves')
          .insert({
            move_no: rollback_move_no,
            move_type: 'transfer',
            status: 'completed',
            notes: `Rollback ${move.move_no} - ย้ายกลับที่เดิม`,
        ...
        if (sourceBalance) {
          ...
          // ลดสต็อกที่ปลายทาง
          const newPieceQty = parseFloat(currentBalance.total_piece_qty || 0) - parseFloat(item.confirmed_piece_qty || 0)
