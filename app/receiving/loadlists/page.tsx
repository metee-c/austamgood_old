...
const LoadlistsPage = () => {
  ...
  const fetchMatchingPreview = async (bfsId: number, picklistId: number | null, faceSheetId: number | null) => {
    ...
    try {
      ...
      // ✅ FIX (edit11): Auto-fill checker จาก loadlist ที่มีอยู่แล้ว
      if (result.checker_employee_id) {
        setBonusFaceSheetCheckers(prev => {
          const updated = { ...prev };
          // Set checker สำหรับ BFS นี้ (ถ้ายังไม่ได้เลือก)
          if (!updated[bfsId]) {
            updated[bfsId] = result.checker_employee_id;
          }
          // ✅ Sync ไปยังทุกแถวที่เลือกไว้ด้วย (ถ้ายังไม่ได้เลือก)
          selectedBonusFaceSheets.forEach(selectedBfsId => {
            if (!updated[selectedBfsId]) {
              updated[selectedBfsId] = result.checker_employee_id;
            }
          });
          return updated;
        });
  ...
  const fetchAllPicklistsForDropdown = async () => {
    try {
      ...
      if (result.data) {
        ...
        // Sort by picklist_code (มากไปน้อย - เรียงตาม string descending)
        picklists.sort((a: any, b: any) => b.picklist_code.localeCompare(a.picklist_code));
  ...
  const fetchOnlineOrdersByShippingProvider = async (shippingProvider: string) => {
    ...
    try {
      ...
      if (result.success && result.data) {
        setAvailableOnlineOrders(result.data);
      } else {
        setAvailableOnlineOrders([]);
      }
    } catch (err) {
  ...
  const handleToggleOnlineOrder = (orderId: string) => {
    setSelectedOnlineOrders(prev =>
  ...
  // ✅ NEW: Toggle all online orders
  const handleToggleAllOnlineOrders = () => {
    if (selectedOnlineOrders.length === availableOnlineOrders.length) {
      setSelectedOnlineOrders([]);
    } else {
      setSelectedOnlineOrders(availableOnlineOrders.map(o => o.id));
    }
  };

  // ✅ NEW: Handler for creating loadlist from online orders
  const handleCreateLoadlistFromOnline = async () => {
    if (selectedOnlineOrders.length === 0) {
      setCreateError('กรุณาเลือกอย่างน้อย 1 ออเดอร์');
      return;
    }
    ...
    try {
      const requestBody = {
        ...
        delivery_number: deliveryNumber || `ONLINE-${Date.now()}`,
      ...
      console.error('Error creating loadlist from online orders:', err);
  ...
  const handleCreateLoadlist = async () => {
    ...
    const hasOnlineOrders = selectedOnlineOrders.length > 0;
    ...
    // ✅ NEW: สร้าง loadlist จากออนไลน์
    if (activeTab === 'online' && hasOnlineOrders) {
      return await handleCreateLoadlistFromOnline();
    }
  ...
  // ฟังก์ชันลบใบโหลดสินค้า
  const handleDeleteLoadlist = async (loadlist: Loadlist) => {
    if (loadlist.status === 'loaded') {
      alert('ไม่สามารถลบใบโหลดที่เสร็จสิ้นแล้วได้');
      return;
    }

    const confirmed = window.confirm(
      `คุณต้องการลบใบโหลด ${loadlist.loadlist_code} ใช่หรือไม่?\n\n` +
      `การกระทำนี้ไม่สามารถเรียกคืนได้ และจะลบการเชื่อมโยงกับใบหยิบ/ใบปะหน้าทั้งหมด`
    );
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.error || 'ลบใบโหลดไม่สำเร็จ');
      }

      // Refresh list
      await fetchLoadlists();
      // Also refresh available picklists/facesheets as they are now free
      fetchAvailablePicklists();
      fetchAvailableFaceSheets();
      fetchAvailableBonusFaceSheets();

    } catch (err: any) {
      console.error('Failed to delete loadlist:', err);
              ...
              <table className="min-w-full border-collapse text-sm table-fixed">
                ...
                <tbody className="bg-white divide-y divide-gray-100 text-[11px]">
                  {filteredLoadlists.slice((currentPage - 1) * pageSize, currentPage * pageSize).map((loadlist) => {
                    ...
                    // ✅ FIX: คอลัม "เลขงานจัดส่ง" - ตรวจสอบ BFS ก่อน เพราะถ้าสร้างจาก BFS แมพ FS ควรแสดงเลข BFS
                    ...
                    } else if (hasPicklists) {
                      // Picklist: แสดงเลขใบหยิบ (picklist_code) แทน delivery_number
                      const picklistCodes = loadlist.picklists.map((pl: any) => pl.picklist_code).filter(Boolean);
                    ...
                    return (
                      <tr key={loadlist.id} className="hover:bg-blue-50/30 transition-colors duration-150">
                        ...
                        {/* คอลัม "OA" - แสดงเลขที่ขึ้นต้นด้วย S */}
                        <td className="px-2 py-0.5 border-r border-gray-100 whitespace-nowrap text-gray-700 font-medium">
                          {loadlist.delivery_number && loadlist.delivery_number.startsWith('S')
                            ...
                            {hasBonusFaceSheets && (() => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`p-1 rounded transition-colors ${isConfirmed
        ...
        <div className="space-y-4">
          ...
          <div className="border-b border-gray-200">
            <div className="flex space-x-4">
              <button
                ...
                className={`px-4 py-2 font-medium text-sm border-b-2 transition-colors ${activeTab === 'picklists'
              ...
              <button
                ...
                className={`px-4 py-2 font-medium text-sm border-b-2 transition-colors ${activeTab === 'face-sheets'
                    ? 'border-green-500 text-green-600'
                    : 'border-transparent text-gray-500 hover:text-gray-700'
                  }`}
              >
              ...
              <button
                ...
                className={`px-4 py-2 font-medium text-sm border-b-2 transition-colors ${activeTab === 'bonus-face-sheets'
                    ? 'border-green-500 text-green-600'
                    : 'border-transparent text-gray-500 hover:text-gray-700'
                  }`}
              >
              ...
              <button
                ...
                className={`px-4 py-2 font-medium text-sm border-b-2 transition-colors ${activeTab === 'online'
                    ? 'border-green-500 text-green-600'
                    : 'border-transparent text-gray-500 hover:text-gray-700'
                  }`}
              >
                <Package className="w-4 h-4 inline-block mr-2" />
                ออนไลน์ ({selectedOnlineOrders.length})
              </button>
            </div>
          </div>
                ...
                <table className="w-full border-collapse text-sm">
                  <thead className="sticky top-0 z-10 bg-gray-100">
                    <tr>
                      ...
                      <th className="px-2 py-2 text-left text-xs font-semibold border-b border-r border-gray-200 whitespace-nowrap">เอกสารจัดสินค้า</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-100 text-[11px]">
                    {!Array.isArray(availablePicklists) || availablePicklists.length === 0 ? (
                      ...
                      availablePicklists.map((picklist) => (
                        <tr
                          key={picklist.id}
                          className={`hover:bg-blue-50/30 transition-colors duration-150 ${selectedPicklists.includes(picklist.id) ? 'bg-green-50' : ''
                            }`}
                        >
                          <td className="px-2 py-0.5 border-r border-gray-100 whitespace-nowrap">
                            <input
                  ...
                  <tbody className="bg-white divide-y divide-gray-100 text-[11px]">
                    {!Array.isArray(availableFaceSheets) || availableFaceSheets.length === 0 ? (
                      ...
                      availableFaceSheets.map((faceSheet, index) => (
                        <tr
                          key={faceSheet.id}
                          className={`hover:bg-blue-50/30 transition-colors duration-150 ${selectedFaceSheets.includes(faceSheet.id) ? 'bg-green-50' : ''
                            }`}
                        >
                          <td className="px-2 py-0.5 border-r border-gray-100 whitespace-nowrap">
                            <input
                          ...
                          unmappedBonusFaceSheets.map((bfs) => {
                            ...
                            return (
                              <React.Fragment key={bfs.id}>
                                <tr
                                  className={`hover:bg-blue-50/30 transition-colors duration-150 ${isSelected ? 'bg-green-50' : ''
                                    }`}
                                >
                                              ...
                                              <tbody className="bg-white">
                                                {orders.map((order: any) => {
                                                  ...
                                                  return (
                                                    <tr
                                                      key={order.order_no}
                                                      className={`hover:bg-blue-50 cursor-pointer ${isOrderSelected ? 'bg-green-50' : ''
                                                        }`}
                          ...
                          availableBonusFaceSheets.map((bonusFaceSheet) => {
                            ...
                            return (
                              <tr
                                ...
                                className={`hover:bg-blue-50/30 transition-colors duration-150 ${isSelected
                                    ? 'bg-green-50'
                                    : isUsed
                              ...
                              >
                                ...
                                <td className="px-2 py-1 border-r border-gray-100">
                                  <select
                                    ...
                                    onChange={(e) => {
                                      ...
                                      // ✅ Sync ค่าไปยังทุกแถวที่เลือกไว้
                                      setBonusFaceSheetMappings(prev => {
                                        const updated = { ...prev };
                                        selectedBonusFaceSheets.forEach(bfsId => {
                                          updated[bfsId] = {
                                            selectedPicklistId: newPicklistId,
                                            selectedFaceSheetId: newPicklistId ? null : prev[bfsId]?.selectedFaceSheetId || null
                                          };
                                        });
                                        return updated;
                                      });
                                      // ✅ เรียก check-matching API สำหรับทุกแถวที่เลือก
                                      selectedBonusFaceSheets.forEach(bfsId => {
                                        fetchMatchingPreview(bfsId, newPicklistId, null);
                                      });
                                    }}
                                    className={`w-full px-2 py-1 border rounded text-xs focus:outline-none focus:ring-1 focus:ring-green-500 ${mapping.selectedFaceSheetId ? 'bg-gray-100 border-gray-200 text-gray-400' : 'border-gray-300'
                                      }`}
                                ...
                                <td className="px-2 py-1 border-r border-gray-100">
                                  <select
                                    ...
                                    onChange={(e) => {
                                      ...
                                      setBonusFaceSheetMappings(prev => {
                                        ...
                                        selectedBonusFaceSheets.forEach(bfsId => {
                                          updated[bfsId] = {
                                            selectedPicklistId: newFaceSheetId ? null : prev[bfsId]?.selectedPicklistId || null,
                                      ...
                                      // ✅ เรียก check-matching API สำหรับทุกแถวที่เลือก
                                      selectedBonusFaceSheets.forEach(bfsId => {
                                        fetchMatchingPreview(bfsId, null, newFaceSheetId);
                                      });
                                    }}
                                    className={`w-full px-2 py-1 border rounded text-xs focus:outline-none focus:ring-1 focus:ring-green-500 ${mapping.selectedPicklistId ? 'bg-gray-100 border-gray-200 text-gray-400' : 'border-gray-300'
                                      }`}
                                  ...
                                  {matchingPreviews[bonusFaceSheet.id] && (
                                    <div className={`mt-1 text-[10px] ${matchingPreviews[bonusFaceSheet.id].loading
                                        ? 'text-gray-500'
                                ...
                                <td className="px-2 py-1 border-r border-gray-100">
                                  <select
                                    ...
                                    onChange={(e) => {
                                      const newCheckerId = e.target.value ? Number(e.target.value) : '';
                                      // ✅ Sync ค่าไปยังทุกแถวที่เลือกไว้
                                      setBonusFaceSheetCheckers(prev => {
                                        const updated = { ...prev };
                                        selectedBonusFaceSheets.forEach(bfsId => {
                                          updated[bfsId] = newCheckerId;
                                        });
                                        return updated;
                                      });
                                    }}
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between bg-gray-50 px-4 py-2 rounded">
                    <label className="flex items-center space-x-2 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={selectedOnlineOrders.length === availableOnlineOrders.length && availableOnlineOrders.length > 0}
                        onChange={handleToggleAllOnlineOrders}
                        className="w-4 h-4 text-green-600 rounded focus:ring-green-500"
                      />
                      <span className="text-sm font-medium text-gray-700">
                        เลือกทั้งหมด ({selectedOnlineOrders.length}/{availableOnlineOrders.length})
                      </span>
                    </label>
                    {loadingOnlineOrders && (
                      <Loader2 className="w-4 h-4 animate-spin text-blue-500" />
                    ...
                    <table className="w-full border-collapse text-sm">
                      ...
                      <tbody className="bg-white divide-y divide-gray-100 text-[11px]">
                        {loadingOnlineOrders ? (
                          ...
                          availableOnlineOrders.map((order) => (
                            <tr
                              key={order.id}
                              className={`hover:bg-blue-50/30 transition-colors duration-150 ${
                                selectedOnlineOrders.includes(order.id) ? 'bg-green-50' : ''
                              }`}
                            >
                              <td className="px-2 py-2 border-r border-gray-100 whitespace-nowrap">
                                <input
                                  type="checkbox"
                                  checked={selectedOnlineOrders.includes(order.id)}
                              ...
                              <td className="px-2 py-2 border-r border-gray-100 whitespace-nowrap text-gray-700">
                                {order.buyer_name}
                              </td>
                              <td className="px-2 py-2 border-r border-gray-100 whitespace-nowrap text-gray-600">
                                {new Date(order.loaded_at).toLocaleString('th-TH')}
                              </td>
                              <td className="px-2 py-2 text-center whitespace-nowrap">
                                <Badge variant="success" size="sm">สแกนขึ้นรถแล้ว</Badge>
                              </td>
                            </tr>
                          ))
                        )}
                  ...
                  {/* Summary */}
                  {selectedOnlineOrders.length > 0 && (
                    <div className="bg-green-50 p-3 rounded-lg text-xs">
                      <div className="font-semibold text-green-700">
                        เลือก {selectedOnlineOrders.length} ออเดอร์
                      </div>
                    </div>
                  )}
                </div>
          ...
          <div className="flex justify-end gap-2 pt-4 border-t">
            ...
            <Button
              ...
              disabled={isCreating || (selectedPicklists.length === 0 && selectedFaceSheets.length === 0 && selectedBonusFaceSheets.length === 0 && selectedOnlineOrders.length === 0)}
              className="bg-green-500 hover:bg-green-600"
            >
              {isCreating ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : null}
              {isCreating ? `กำลังสร้าง...` : `สร้าง (${selectedPicklists.length + selectedFaceSheets.length + selectedBonusFaceSheets.length + selectedOnlineOrders.length} รายการ)`}
            </Button>
          </div>
        </div>
      </Modal>
        ...
        <div className="space-y-4">
                ...
                {mappedFaceSheets.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="px-4 py-8 text-center text-gray-500">
                      <div className="flex flex-col items-center">
                        ...
                        <p className="font-medium text-gray-700">ใบปะหน้าของแถมนี้ไม่ได้แมพกับเอกสารใดๆ</p>
                        <p className="text-xs text-gray-500 mt-1">สามารถพิมพ์ใบเช็คของแถมได้โดยตรงจากปุ่ม &quot;พิมพ์ทั้งหมด&quot; ด้านล่าง</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                  mappedFaceSheets.map((fs) => (
                    <tr
                      ...
                      className={`hover:bg-blue-50/30 transition-colors duration-150 ${selectedFaceSheetsForPrint.includes(fs.face_sheet_id) ? 'bg-green-50' : ''
                        }`}
                    >
                      <td className="px-2 py-1 border-r border-gray-100">
                        <input
          ...
          <div className="flex gap-3 justify-between pt-4 border-t">
            <Button
              variant="outline"
              onClick={() => {
                // ✅ FIX (edit11): ปริ้นใบเช็คของแถมตาม matched_package_ids (ส่ง loadlist_id)
                if (bonusFaceSheetToPrint && currentLoadlistIdForPrint) {
                  window.open(
                    `/api/bonus-face-sheets/print?id=${bonusFaceSheetToPrint.id}&loadlist_id=${currentLoadlistIdForPrint}`,
                    '_blank'
