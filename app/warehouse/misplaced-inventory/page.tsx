  ...
  const handleExport = async () => {
    try {
      ...
      const exportData = result.data.map((row: MisplacedItem) => ({
        ...
        'Lot No': row.lot_no || '-',
        'วันผลิต': row.production_date ? new Date(row.production_date).toLocaleDateString('en-GB') : '-',
        'วันหมดอายุ': row.expiry_date ? new Date(row.expiry_date).toLocaleDateString('en-GB') : '-',
