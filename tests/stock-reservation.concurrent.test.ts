 ...
 * 
 * Run: npm test -- tests/stock-reservation.concurrent.test.ts
 * 
 * Requirements:
 * - Jest + ts-jest
...
async function createTestSku(
  ...
  // Create test SKU
  await supabase.from('wms_skus').insert({
    sku_id: skuId,
    sku_name: `Test SKU ${skuId}`,
    sku_type: 'finished_goods',
    uom: 'piece',
    qty_per_pack: 1,
    preparation_area: 'PK',
  });
...
async function cleanupTestData(supabase: SupabaseClient, skuId: string) {
  try {
    ...
    await supabase.from('face_sheet_item_reservations').delete().like('face_sheet_item_id', '%');
    await supabase.from('face_sheet_items').delete().match({ sku_id: skuId });
    await supabase.from('face_sheets').delete().like('face_sheet_code', 'FS-%');
    await supabase.from('wms_order_items').delete().match({ sku_id: skuId });
    await supabase.from('wms_orders').delete().like('order_no', 'TEST-ORD-%');
    await supabase.from('wms_inventory_balances').delete().match({ sku_id: skuId });
    await supabase.from('wms_skus').delete().match({ sku_id: skuId });
  } catch (error) {
...
describe('Stock Reservation - Concurrent Tests', () => {
  ...
  beforeAll(async () => {
    ...
    // Create test SKU
    await createTestSku(supabase, TEST_CONFIG.testSku);
  ...
  describe('FEFO/FIFO Ordering', () => {
    ...
    beforeAll(async () => {
      // Create test SKU
      await createTestSku(supabase, skuForFefo);
      ...
      await supabase.from('wms_inventory_balances').insert({
        ...
        total_piece_qty: 50,
        reserved_piece_qty: 0,
        expiry_date: new Date(today.getTime() + 7 * 24 * 60 * 60 * 1000).toISOString(), // +7 days
        production_date: new Date(today.getTime() - 30 * 24 * 60 * 60 * 1000).toISOString(), // -30 days
      });
      ...
      await supabase.from('wms_inventory_balances').insert({
        ...
        total_piece_qty: 50,
        reserved_piece_qty: 0,
        expiry_date: new Date(today.getTime() + 30 * 24 * 60 * 60 * 1000).toISOString(), // +30 days
        production_date: new Date(today.getTime() - 15 * 24 * 60 * 60 * 1000).toISOString(), // -15 days
      });
    });
  ...
  describe('Load Testing', () => {
    ...
    beforeAll(async () => {
      // Create test SKU
      await createTestSku(supabase, skuForLoad);
