...
-- Description: แก้ไข type error ใน LPAD function call
-- 
-- BUG: function lpad(integer, integer, unknown) does not exist
-- 
-- สาเหตุ:
-- - v_package_number เป็น INTEGER
-- - LPAD(v_package_number::TEXT, 3, '0') ทำให้ PostgreSQL สับสนกับ function signature
-- - ต้อง cast เป็น TEXT อย่างชัดเจนก่อนส่งเข้า LPAD
-- 
-- แก้ไข:
-- - ใช้ LPAD(v_package_number::TEXT, 3, '0') แทน
-- - หรือใช้ LPAD(CAST(v_package_number AS TEXT), 3, '0')
-- ============================================================================
...
'สร้างใบปะหน้าของแถมพร้อมจองสต็อคแบบ atomic - แก้ไข LPAD type error (Migration 246)';
