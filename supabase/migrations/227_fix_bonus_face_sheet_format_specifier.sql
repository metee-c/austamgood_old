-- ============================================
-- Migration: แก้ไข format() specifier ใน reserve_stock_for_bonus_face_sheet_items
-- Error: unrecognized format() type specifier "."
-- Reason: PostgreSQL format() ไม่รองรับ %.2f (ต้องใช้ %s และ ROUND/TO_CHAR)
-- ============================================
...
BEGIN
  ...
  LOOP
    ...
    IF v_remaining_qty > 0 THEN
      ...
      -- ✅ FIX: ใช้ %s แทน %.2f และใช้ ROUND() เพื่อจำกัดทศนิยม
      RETURN QUERY SELECT
        false,
        0,
        v_items_count,
        format('สต็อกในบ้านหยิบไม่เพียงพอสำหรับ SKU: %s (ขาด %s ชิ้น) กรุณาเติมสต็อกที่บ้านหยิบก่อน',
               v_item.sku_id, ROUND(v_remaining_qty, 2)::TEXT);
      RETURN;
    END IF;
  END LOOP;
...
FIXED: format() specifier %.2f → %s with ROUND()';
