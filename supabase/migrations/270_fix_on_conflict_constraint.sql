-- ============================================================================
-- Migration: 270_fix_on_conflict_constraint.sql
-- Description: แก้ไข ON CONFLICT ให้ตรงกับ unique constraint ที่มีอยู่
--
-- BUG: ON CONFLICT (warehouse_id, sku_id, location_id, COALESCE(pallet_id, ''))
--      ไม่ตรงกับ unique index/constraint ใดๆ ที่มีอยู่
--
-- FIX: ใช้ upsert_or_update pattern แทน ON CONFLICT
-- ============================================================================
...
'จองสต็อคสำหรับใบปะหน้า - ใช้ SELECT-then-INSERT/UPDATE แทน ON CONFLICT (Fix: constraint mismatch)';
