-- Migration: Fix balance sync trigger to handle transfer transactions
-- Description: Remove 'transfer' from the skip list so it updates inventory balances
...
BEGIN
    -- ข้าม transaction types ที่โค้ดจัดการ balance เอง
    -- ลบ 'transfer' ออกเพื่อให้ trigger จัดการ balance สำหรับการย้ายสินค้า
    IF NEW.transaction_type IN ('putaway', 'replenishment', 'adjustment') THEN
        RETURN NEW;
    END IF;
...
COMMENT ON FUNCTION sync_inventory_ledger_to_balance() IS 'Sync inventory ledger to balances - now handles transfer transactions';
