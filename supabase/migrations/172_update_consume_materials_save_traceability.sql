-- Migration: Update consume_materials_on_production_receive to save traceability data
-- บันทึก pallet_id, material_production_date, material_expiry_date ลง production_receipt_materials
...
BEGIN
    ...
    LOOP
        ...
        -- *** บันทึก traceability data ลง production_receipt_materials ***
        UPDATE production_receipt_materials
        SET 
            pallet_id = v_pallet_id,
            material_production_date = v_production_date,
            material_expiry_date = v_expiry_date,
            updated_at = CURRENT_TIMESTAMP
        WHERE id = v_material.prm_id;
...
- บันทึก traceability data (pallet_id, material_production_date, material_expiry_date) ลง production_receipt_materials
- เพื่อให้ ledger entry match กับ balance ได้ถูกต้อง (migration 172)';
