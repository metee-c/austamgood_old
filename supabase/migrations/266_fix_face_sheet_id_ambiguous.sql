-- ============================================================================
-- Migration: 266_fix_face_sheet_id_ambiguous.sql
-- Description: แก้ไข error "column reference face_sheet_id is ambiguous"
--
-- BUG: face_sheet_id ถูกใช้ทั้งใน face_sheet_items table และ CTE
-- FIX: ใช้ v_face_sheet_id variable โดยตรงแทน
-- ============================================================================
...
BEGIN
    ...
    -- ✅ FIX: Use fs_id alias to avoid ambiguity with return column
    -- ========================================
    ...
    package_data AS (
        SELECT
            ...
            np.shipping_address as address,
            np.province,
            np.contact_person as contact_name,
            np.phone,
            np.hub,
            np.order_item_id,
            np.order_qty,
    ...
    -- Insert face_sheet_items - one per package
    ...
    -- ✅ FIX: Count packages using v_face_sheet_id variable directly
    SELECT
        COUNT(*),
...
'สร้างใบปะหน้าพร้อมจองสต็อคแบบ atomic - สร้าง 1 package ต่อ 1 pack (Fix: face_sheet_id ambiguous)';
