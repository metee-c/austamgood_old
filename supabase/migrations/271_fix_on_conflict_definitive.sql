-- ============================================================================
-- Migration: 271_fix_on_conflict_definitive.sql
-- Description: แก้ไข ON CONFLICT ให้ใช้ SELECT-then-INSERT/UPDATE pattern
--              Drop function ก่อนแล้ว Create ใหม่เพื่อให้แน่ใจว่าใช้ version ใหม่
--
-- BUG: ON CONFLICT (warehouse_id, sku_id, location_id, COALESCE(pallet_id, ''))
--      ไม่ตรงกับ unique constraint ใดๆ ที่มีอยู่ใน wms_inventory_balances
--
-- Available unique constraints:
-- 1. uq_wms_inventory_balances_sku_location: (warehouse_id, location_id, sku_id, pallet_id) - no COALESCE
-- 2. wms_inventory_balances_unique_idx: (warehouse_id, location_id, sku_id,
--    COALESCE(production_date, '1900-01-01'), COALESCE(expiry_date, '1900-01-01'), COALESCE(lot_no, ''))
--
-- FIX: ใช้ SELECT-then-INSERT/UPDATE pattern แทน ON CONFLICT
-- ============================================================================
...
BEGIN
    ...
    FOR v_item IN
        SELECT
            ...
            ms.qty_per_pack,
            COALESCE(pa.location_id, 'PK001') as prep_area_location
        ...
        LEFT JOIN master_customer mc ON mc.customer_id = fsp.customer_id
        LEFT JOIN preparation_areas pa ON pa.hub = mc.hub
    ...
    LOOP
        ...
        IF v_qty_reserved < v_qty_to_reserve THEN
            ...
            BEGIN
                ...
                -- ✅ FIX: Use SELECT then INSERT/UPDATE instead of ON CONFLICT
                -- Check if virtual balance exists
        ...
        -- Update face_sheet_item with reservation info
...
'จองสต็อคสำหรับใบปะหน้า - ใช้ SELECT-then-INSERT/UPDATE แทน ON CONFLICT (Fix: constraint mismatch - Migration 271)';
