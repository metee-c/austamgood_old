-- ============================================================================
-- Migration: 314_update_route_stop_on_item_rollback.sql
-- Description: อัปเดต route stop เมื่อ rollback order item
-- Date: 2026-01-29
-- ============================================================================
...
BEGIN
    ...
    -- เก็บข้อมูลสำหรับอัปเดต route stop
    v_item_qty := COALESCE(v_order_item.order_qty, 0);
    v_item_weight := COALESCE(v_order_item.order_weight, 0);
    ...
    FOR v_picklist_item IN
        ...
        FOR UPDATE
    LOOP
        -- Void picklist item (uses void_reason column)
        ...
        -- Release reservation (skip released_by since it's UUID type)
    ...
    FOR v_fs_item IN
        ...
        FOR UPDATE
    LOOP
        -- Void face sheet item (uses void_reason column, not voided_reason)
    ...
    FOR v_bonus_item IN
        ...
        FOR UPDATE
    LOOP
        -- Void bonus face sheet item (uses void_reason column, not voided_reason)
    ...
    -- ========================================
    -- STEP 6: UPDATE ROUTE STOP (ถ้ามี)
    -- ========================================
    
    -- อัปเดต route stop ที่เกี่ยวข้องกับ order นี้
    UPDATE receiving_route_stops
    SET load_units = GREATEST(0, COALESCE(load_units, 0) - v_item_qty),
        load_weight_kg = GREATEST(0, COALESCE(load_weight_kg, 0) - v_item_weight),
        updated_at = NOW(),
        notes = COALESCE(notes, '') || ' | Item Rollback: ' || v_order_item.sku_id
    WHERE order_id = v_order_item.order_id
       OR (tags->'order_ids')::jsonb @> to_jsonb(v_order_item.order_id);
    
    -- อัปเดต route plan input (ถ้ามี)
    UPDATE receiving_route_plan_inputs
    SET demand_units = GREATEST(0, COALESCE(demand_units, 0) - v_item_qty),
        demand_weight_kg = GREATEST(0, COALESCE(demand_weight_kg, 0) - v_item_weight),
        updated_at = NOW()
    WHERE order_id = v_order_item.order_id;
    ...
    INSERT INTO wms_rollback_audit_logs (
        ...
        jsonb_build_object(
            ...
            'route_stop_updated', true,
            'qty_reduced', v_item_qty,
            'weight_reduced', v_item_weight
        ),
        'completed',
        NOW()
    );
...
DO $$
BEGIN
    RAISE NOTICE '✅ Migration 314 completed: execute_order_item_rollback now updates route stops';
END $$;
