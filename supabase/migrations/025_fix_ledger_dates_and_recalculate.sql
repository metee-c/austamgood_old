-- Migration: Fix production_date and expiry_date in ledger, then recalculate balances
-- Description: Update ledger entries to have correct dates from balance, then rebuild balances
-- Date: 2025-01-22

-- Step 1: Update ledger entries to have production_date and expiry_date from balance
...
WHERE l.warehouse_id = b.warehouse_id
  ...
  AND l.pallet_id = b.pallet_id
  AND (l.production_date IS NULL OR l.expiry_date IS NULL)
...
BEGIN
    ...
    RAISE NOTICE 'Ledger dates fixed and balances recalculated:';
