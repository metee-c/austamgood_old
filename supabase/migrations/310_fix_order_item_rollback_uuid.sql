-- ============================================================================
-- Migration: 310_fix_order_item_rollback_uuid.sql
-- Description: แก้ไข function ให้ใช้ UUID สำหรับ released_by
-- Date: 2026-01-29
-- ============================================================================
...
DECLARE
    ...
    v_user_uuid UUID;
BEGIN
    -- Get user UUID from master_system_user
    ...
    LOOP
        ...
        -- Release reservation (use UUID for released_by)
        UPDATE picklist_item_reservations
        SET status = 'released',
            released_at = NOW(),
            released_by = v_user_uuid,
...
DO $$
BEGIN
    RAISE NOTICE '✅ Migration 310 completed: execute_order_item_rollback function fixed for UUID';
END $$;
