-- ============================================================================
-- Migration: 273_fix_preparation_area_table_name.sql
-- Description: แก้ไขชื่อตารางจาก preparation_areas เป็น preparation_area
--
-- BUG: ใน reserve_stock_for_face_sheet_items ใช้ชื่อตาราง preparation_areas
--      แต่ตารางที่ถูกต้องคือ preparation_area (ไม่มี s)
--
-- FIX: เปลี่ยนเป็น preparation_area และ join ผ่าน hub
-- ============================================================================
...
BEGIN
    ...
    FOR v_item IN
        SELECT
            ...
            ms.qty_per_pack,
            -- ✅ FIX: Use preparation_area (not preparation_areas) and join via hub
            COALESCE(pa.area_code, 'PK001') as prep_area_location
        ...
        -- ✅ FIX: Changed table name from preparation_areas to preparation_area
        -- Join on hub field which exists in preparation_area table
        LEFT JOIN preparation_area pa ON pa.hub = mc.hub AND pa.warehouse_id = p_warehouse_id AND pa.status = 'active'
...
'จองสต็อคสำหรับใบปะหน้า - Fix: preparation_area table name (Migration 273)';
