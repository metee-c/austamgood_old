-- ============================================================================
-- Migration: 249_fix_bonus_face_sheet_remove_exception_vars.sql
-- Description: ลบ PG_EXCEPTION_DETAIL และ PG_EXCEPTION_HINT ที่ทำให้เกิด error
-- 
-- BUG: column "pg_exception_detail" does not exist
-- 
-- สาเหตุ:
-- - PG_EXCEPTION_DETAIL และ PG_EXCEPTION_HINT ไม่ทำงานใน EXCEPTION block
-- - ใน PostgreSQL, variables เหล่านี้ใช้ได้เฉพาะใน GET DIAGNOSTICS
-- 
-- แก้ไข:
-- - ลบ PG_EXCEPTION_DETAIL และ PG_EXCEPTION_HINT ออก
-- - ใช้เฉพาะ SQLSTATE และ SQLERRM ซึ่งทำงานได้เสมอ
-- ============================================================================
...
EXCEPTION
    WHEN OTHERS THEN
        -- ✅ FIX: ลบ PG_EXCEPTION_DETAIL และ PG_EXCEPTION_HINT ออก
        -- ใช้เฉพาะ SQLSTATE และ SQLERRM ซึ่งทำงานได้เสมอ
        RETURN QUERY SELECT 
            FALSE,
            NULL::BIGINT,
...
'สร้างใบปะหน้าของแถมพร้อมจองสต็อคแบบ atomic - ลบ PG_EXCEPTION variables (Migration 249)';
