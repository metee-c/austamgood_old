...
COMMENT ON TYPE "public"."picklist_status_enum" IS 'สถานะของใบจัดเก็บสินค้า (pending, assigned, picking, completed, cancelled)';
...
COMMENT ON TYPE "public"."receiving_route_plan_status_enum" IS 'สถานะของชุดแผนเส้นทางรับสินค้า (draft/optimizing/published/completed/cancelled)';
...
CREATE OR REPLACE FUNCTION "public"."calculate_freight_derived_prices"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
    -- Calculate price per km
    IF NEW.total_distance_km > 0 THEN
        NEW.calculated_price_per_km = NEW.base_price / NEW.total_distance_km;
    END IF;
...
ALTER FUNCTION "public"."calculate_freight_derived_prices"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."create_face_sheet_packages"("p_face_sheet_no" character varying DEFAULT NULL::character varying, "p_warehouse_id" character varying DEFAULT 'WH01'::character varying, "p_created_by" character varying DEFAULT 'System'::character varying, "p_delivery_date" "date" DEFAULT NULL::"date") RETURNS TABLE("success" boolean, "face_sheet_id" bigint, "face_sheet_no" character varying, "total_packages" integer, "small_size_count" integer, "large_size_count" integer, "message" "text")
...
ALTER FUNCTION "public"."create_face_sheet_packages"("p_face_sheet_no" character varying, "p_warehouse_id" character varying, "p_created_by" character varying, "p_delivery_date" "date") OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."master_freight_rate" (
    ...
    "calculated_price_per_km" numeric(10,2),
    "calculated_price_per_kg" numeric(10,2),
    "calculated_price_per_pallet" numeric(10,2),
    ...
    CONSTRAINT "chk_fuel_surcharge_rate" CHECK ((("fuel_surcharge_rate" IS NULL) OR ("fuel_surcharge_rate" >= (0)::numeric))),
    ...
    CONSTRAINT "chk_positive_min_charge" CHECK ((("min_charge" IS NULL) OR ("min_charge" > (0)::numeric))),
...
COMMENT ON COLUMN "public"."master_freight_rate"."min_charge" IS 'Minimum charge (if applicable)';



COMMENT ON COLUMN "public"."master_freight_rate"."calculated_price_per_km" IS 'Calculated price per kilometer';



COMMENT ON COLUMN "public"."master_freight_rate"."calculated_price_per_kg" IS 'Calculated price per kilogram';



COMMENT ON COLUMN "public"."master_freight_rate"."calculated_price_per_pallet" IS 'Calculated price per pallet';



COMMENT ON COLUMN "public"."master_freight_rate"."fuel_surcharge_rate" IS 'Fuel surcharge rate percentage';
...
CREATE OR REPLACE VIEW "public"."active_freight_rates" AS
 SELECT "freight_rate_id",
    ...
    "price_unit",
    "min_charge",
    "calculated_price_per_km",
    "calculated_price_per_kg",
    "calculated_price_per_pallet",
    "fuel_surcharge_rate",
    ...
    "updated_at",
        CASE
            WHEN (("effective_end_date" IS NULL) OR ("effective_end_date" >= CURRENT_DATE)) THEN true
            ELSE false
        END AS "is_active",
        CASE
            WHEN ("total_distance_km" > (0)::numeric) THEN "round"(("base_price" / "total_distance_km"), 2)
            ELSE NULL::numeric
        END AS "price_per_km_actual"
   FROM "public"."master_freight_rate" "fr"
  WHERE (("effective_start_date" <= CURRENT_DATE) AND (("effective_end_date" IS NULL) OR ("effective_end_date" >= CURRENT_DATE)));
...
CREATE TABLE IF NOT EXISTS "public"."face_sheets" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"(),
    "updated_at" timestamp with time zone DEFAULT "now"(),
    CONSTRAINT "face_sheets_status_check" CHECK ((("status")::"text" = ANY ((ARRAY['draft'::character varying, 'generated'::character varying, 'printed'::character varying, 'completed'::character varying])::"text"[])))
);
...
CREATE TABLE IF NOT EXISTS "public"."load_list_items" (
    "id" bigint NOT NULL,
    "load_list_id" bigint NOT NULL,
...
ALTER TABLE "public"."load_list_items" OWNER TO "postgres";


COMMENT ON TABLE "public"."load_list_items" IS 'ตารางรายการใบจัดสินค้าที่ต้องโหลดในแต่ละใบโหลด';



COMMENT ON COLUMN "public"."load_list_items"."load_list_id" IS 'FK อ้างอิงถึงใบโหลดในตาราง load_lists';



COMMENT ON COLUMN "public"."load_list_items"."picklist_id" IS 'FK อ้างอิงถึงใบจัดสินค้าที่ต้องโหลด';



COMMENT ON COLUMN "public"."load_list_items"."sequence" IS 'ลำดับการโหลด';



COMMENT ON COLUMN "public"."load_list_items"."loaded_at" IS 'เวลาที่โหลดเสร็จ';



CREATE SEQUENCE IF NOT EXISTS "public"."load_list_items_id_seq"
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER SEQUENCE "public"."load_list_items_id_seq" OWNER TO "postgres";


ALTER SEQUENCE "public"."load_list_items_id_seq" OWNED BY "public"."load_list_items"."id";



CREATE TABLE IF NOT EXISTS "public"."load_lists" (
    "id" bigint NOT NULL,
    "load_list_no" character varying(50) NOT NULL,
    "load_date" "date" NOT NULL,
    "vehicle_id" bigint,
    "driver_employee_id" bigint,
    "status" "public"."load_list_status_enum" DEFAULT 'draft'::"public"."load_list_status_enum" NOT NULL,
    "total_picklists" integer DEFAULT 0,
    "total_items" integer DEFAULT 0,
    ...
    CONSTRAINT "load_lists_total_items_check" CHECK (("total_items" >= 0)),
    CONSTRAINT "load_lists_total_picklists_check" CHECK (("total_picklists" >= 0)),
    CONSTRAINT "load_lists_total_quantity_check" CHECK (("total_quantity" >= (0)::numeric))
);


ALTER TABLE "public"."load_lists" OWNER TO "postgres";


COMMENT ON TABLE "public"."load_lists" IS 'ตารางหลักสำหรับใบโหลดสินค้าขึ้นรถ';



COMMENT ON COLUMN "public"."load_lists"."load_list_no" IS 'รหัสใบโหลดที่ไม่ซ้ำกัน เช่น LL-20251023-001';



COMMENT ON COLUMN "public"."load_lists"."load_date" IS 'วันที่โหลดสินค้า';



COMMENT ON COLUMN "public"."load_lists"."vehicle_id" IS 'FK อ้างอิงถึงรถที่จะโหลดสินค้า';



COMMENT ON COLUMN "public"."load_lists"."driver_employee_id" IS 'FK อ้างอิงถึงพนักงานขับรถ';



COMMENT ON COLUMN "public"."load_lists"."status" IS 'สถานะปัจจุบันของใบโหลด';



COMMENT ON COLUMN "public"."load_lists"."total_picklists" IS 'จำนวนใบจัดทั้งหมดที่ต้องโหลด';



COMMENT ON COLUMN "public"."load_lists"."total_items" IS 'จำนวนรายการสินค้าทั้งหมด';



COMMENT ON COLUMN "public"."load_lists"."total_quantity" IS 'จำนวนชิ้นทั้งหมด';



CREATE SEQUENCE IF NOT EXISTS "public"."load_lists_id_seq"
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER SEQUENCE "public"."load_lists_id_seq" OWNER TO "postgres";


ALTER SEQUENCE "public"."load_lists_id_seq" OWNED BY "public"."load_lists"."id";
...
CREATE TABLE IF NOT EXISTS "public"."master_location" (
    ...
    CONSTRAINT "master_location_location_type_check" CHECK ((("location_type")::"text" = ANY ((ARRAY['rack'::character varying, 'floor'::character varying, 'bulk'::character varying, 'other'::character varying, 'receiving'::character varying])::"text"[])))
);
...
COMMENT ON COLUMN "public"."master_location"."location_type" IS 'Location type (rack/floor/bulk/other)';
...
COMMENT ON TABLE "public"."receiving_route_plans" IS 'หัวตารางแผนเส้นทางรับสินค้า ใช้เก็บการตั้งค่าและผลลัพธ์รวมของการจัดเส้นทางแบบ VRP';
...
COMMENT ON TABLE "public"."receiving_route_stops" IS 'รายละเอียดจุดแวะของแต่ละเส้นทางรับสินค้า รวมสถานะ เวลา และโหลดที่ต้องรับ';
...
COMMENT ON TABLE "public"."receiving_route_trips" IS 'ใบงานเส้นทางรับสินค้าแต่ละคันรถ/เที่ยว พร้อมข้อมูลต้นทุนและเวลาที่ใช้';
...
COMMENT ON TABLE "public"."wms_inventory_balances" IS 'ตารางคงเหลือสินค้าปัจจุบันตามตำแหน่งจัดเก็บ';
...
COMMENT ON TABLE "public"."wms_inventory_ledger" IS 'ประวัติการเคลื่อนไหวของสต็อก (Stock Movement Ledger)';
...
ALTER TABLE ONLY "public"."load_list_items" ALTER COLUMN "id" SET DEFAULT "nextval"('"public"."load_list_items_id_seq"'::"regclass");



ALTER TABLE ONLY "public"."load_lists" ALTER COLUMN "id" SET DEFAULT "nextval"('"public"."load_lists_id_seq"'::"regclass");
...
ALTER TABLE ONLY "public"."load_list_items"
    ADD CONSTRAINT "load_list_items_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."load_lists"
    ADD CONSTRAINT "load_lists_load_list_no_key" UNIQUE ("load_list_no");



ALTER TABLE ONLY "public"."load_lists"
    ADD CONSTRAINT "load_lists_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."load_list_items"
    ADD CONSTRAINT "uq_load_list_picklist" UNIQUE ("load_list_id", "picklist_id");
...
CREATE INDEX "idx_load_list_items_load_list" ON "public"."load_list_items" USING "btree" ("load_list_id");



CREATE INDEX "idx_load_list_items_picklist" ON "public"."load_list_items" USING "btree" ("picklist_id");



CREATE INDEX "idx_load_lists_load_date" ON "public"."load_lists" USING "btree" ("load_date");



CREATE INDEX "idx_load_lists_status" ON "public"."load_lists" USING "btree" ("status");



CREATE INDEX "idx_load_lists_vehicle" ON "public"."load_lists" USING "btree" ("vehicle_id");
...
CREATE OR REPLACE TRIGGER "calculate_freight_prices_trigger" BEFORE INSERT OR UPDATE ON "public"."master_freight_rate" FOR EACH ROW EXECUTE FUNCTION "public"."calculate_freight_derived_prices"();



CREATE OR REPLACE TRIGGER "handle_updated_at_load_lists" BEFORE UPDATE ON "public"."load_lists" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at_column"();
...
ALTER TABLE ONLY "public"."load_list_items"
    ADD CONSTRAINT "fk_load_list_items_employee" FOREIGN KEY ("loaded_by_employee_id") REFERENCES "public"."master_employee"("employee_id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."load_list_items"
    ADD CONSTRAINT "fk_load_list_items_load_list" FOREIGN KEY ("load_list_id") REFERENCES "public"."load_lists"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."load_list_items"
    ADD CONSTRAINT "fk_load_list_items_picklist" FOREIGN KEY ("picklist_id") REFERENCES "public"."picklists"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."load_lists"
    ADD CONSTRAINT "fk_load_lists_driver" FOREIGN KEY ("driver_employee_id") REFERENCES "public"."master_employee"("employee_id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."load_lists"
    ADD CONSTRAINT "fk_load_lists_vehicle" FOREIGN KEY ("vehicle_id") REFERENCES "public"."master_vehicle"("vehicle_id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."load_lists"
    ADD CONSTRAINT "load_lists_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "auth"."users"("id");
...
CREATE POLICY "Allow all for authenticated users" ON "public"."load_list_items" TO "authenticated" USING (true) WITH CHECK (true);



CREATE POLICY "Allow all for authenticated users" ON "public"."load_lists" TO "authenticated" USING (true) WITH CHECK (true);
...
ALTER TABLE "public"."load_list_items" ENABLE ROW LEVEL SECURITY;


ALTER TABLE "public"."load_lists" ENABLE ROW LEVEL SECURITY;
...
GRANT ALL ON FUNCTION "public"."calculate_freight_derived_prices"() TO "anon";
GRANT ALL ON FUNCTION "public"."calculate_freight_derived_prices"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."calculate_freight_derived_prices"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."create_face_sheet_packages"("p_face_sheet_no" character varying, "p_warehouse_id" character varying, "p_created_by" character varying, "p_delivery_date" "date") TO "anon";
GRANT ALL ON FUNCTION "public"."create_face_sheet_packages"("p_face_sheet_no" character varying, "p_warehouse_id" character varying, "p_created_by" character varying, "p_delivery_date" "date") TO "authenticated";
GRANT ALL ON FUNCTION "public"."create_face_sheet_packages"("p_face_sheet_no" character varying, "p_warehouse_id" character varying, "p_created_by" character varying, "p_delivery_date" "date") TO "service_role";
...
GRANT ALL ON TABLE "public"."load_list_items" TO "anon";
GRANT ALL ON TABLE "public"."load_list_items" TO "authenticated";
GRANT ALL ON TABLE "public"."load_list_items" TO "service_role";



GRANT ALL ON SEQUENCE "public"."load_list_items_id_seq" TO "anon";
GRANT ALL ON SEQUENCE "public"."load_list_items_id_seq" TO "authenticated";
GRANT ALL ON SEQUENCE "public"."load_list_items_id_seq" TO "service_role";



GRANT ALL ON TABLE "public"."load_lists" TO "anon";
GRANT ALL ON TABLE "public"."load_lists" TO "authenticated";
GRANT ALL ON TABLE "public"."load_lists" TO "service_role";



GRANT ALL ON SEQUENCE "public"."load_lists_id_seq" TO "anon";
GRANT ALL ON SEQUENCE "public"."load_lists_id_seq" TO "authenticated";
GRANT ALL ON SEQUENCE "public"."load_lists_id_seq" TO "service_role";
