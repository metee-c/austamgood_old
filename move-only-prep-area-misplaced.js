/**
 * ย้ายเฉพาะสินค้าที่อยู่ในบ้านหยิบ (preparation areas) ที่วางผิดตำแหน่ง
 * ไม่ย้ายสินค้าจากโลเคชั่นเก็บปกติ
 */
...
async function main() {
  console.log('🔧 กำลังย้ายสินค้าที่อยู่ในบ้านหยิบผิดตำแหน่ง...\n')

  // ดึงรายการ preparation areas ทั้งหมด
  ...
  console.log(`📍 พบบ้านหยิบทั้งหมด: ${prepAreaCodes.length} แห่ง\n`)

  // ดึงสินค้าที่อยู่ในบ้านหยิบเท่านั้น
  const { data: inventoryInPrepAreas, error } = await supabase
  ...
  // กรองเฉพาะสินค้าที่อยู่ผิดบ้านหยิบ
  const itemsToMove = inventoryInPrepAreas.filter(item => {
  ...
  console.log(`📊 พบสินค้าที่อยู่ผิดบ้านหยิบ: ${itemsToMove.length} รายการ\n`)

  if (itemsToMove.length === 0) {
    console.log('✅ ไม่มีสินค้าที่ต้องย้าย ทุกอย่างอยู่ในบ้านหยิบที่ถูกต้องแล้ว')
    return
  }
    ...
    try {
      ...
      const { data: move } = await supabase
        ...
        .insert({
          ...
          move_type: 'transfer',
          status: 'completed',
          notes: `ย้ายให้ตรงกับ default_location (เฉพาะบ้านหยิบ) - Balance ID: ${item.balance_id}`,
