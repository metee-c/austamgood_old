# แก้ไขปัญหา UUID Error ในการย้ายสินค้า

## ปัญหา
```
invalid input syntax for type uuid: "34698"
```

เกิดจาก `balance_id` ใน trigger ถูกประกาศเป็น `UUID` แต่ในตารางเป็น `bigint`

## วิธีแก้ไข

### วิธีที่ 1: รัน SQL ผ่าน Supabase Dashboard (ง่ายที่สุด)

1. เปิด Supabase Dashboard → SQL Editor
2. คัดลอกและวาง SQL ข้างล่างนี้:
```sql
  ...
  ELSE
    ...
    IF NEW.direction = 'in' THEN
      INSERT INTO wms_inventory_balances (
        ...
        0, 0, NEW.movement_at, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NEW.lot_no
      );
    END IF;
  END IF;
...
4. ลองย้ายสินค้าใหม่ที่ http://localhost:3000/mobile/transfer

### วิธีที่ 2: ใช้ไฟล์ SQL ที่สร้างไว้
```bash
psql -h YOUR_HOST -U YOUR_USER -d YOUR_DATABASE -f fix-uuid-issue.sql
```

## ตรวจสอบผลลัพธ์
หลังแก้ไขแล้ว การย้ายสินค้าควรทำงานได้ปกติ ไม่มี error `invalid input syntax for type uuid`
