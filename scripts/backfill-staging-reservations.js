/**
 * Backfill Script: สร้าง staging reservations สำหรับข้อมูลเก่า
 * 
 ...
 * 
 * Usage: node scripts/backfill-staging-reservations.js
 */
  ...
  for (const item of items || []) {
    try {
      ...
      if (balanceError || !balance) {
        console.warn(`⚠️  No balance found for picklist item ${item.id} at ${stagingLocation}`);
        failed++;
        continue;
      }
...
async function backfillFaceSheetReservations() {
  ...
  for (const item of items || []) {
    try {
      ...
      if (balanceError || !balance) {
        console.warn(`⚠️  No balance found for face sheet item ${item.id} at ${stagingLocation}`);
        failed++;
        continue;
      }
...
async function backfillBonusFaceSheetReservations() {
  ...
  for (const item of items || []) {
    try {
      ...
      if (balanceError || !balance) {
        console.warn(`⚠️  No balance found for bonus face sheet item ${item.id} at ${stagingLocation}`);
        failed++;
        continue;
      }
 ...
 */
async function main() {
  console.log('🚀 Starting Staging Reservation Backfill Script');
  console.log('================================================\n');
